class Solution:
    def findDuplicateSubtrees(self, root):
        ...
        """
        unique_elements = {}
        self.inorder(root, unique_elements)
        self.filter_only_one_occurance(unique_elements)
        # print(unique_elements)
        return self.combinations(unique_elements)
        
    def isMirror(self, r1, r2, structure):
        if r1 == None and r2!= None:
            ...
            if r1.val == r2.val:
                structure.append(r1.val)
                return self.isMirror(r1.left, r2.left, structure) and self.isMirror(r1.right, r2.right, structure)
            else:
                return False

    
    def combinations(self, unique_elements):
        all_unique_structures = {}
        for node_list in unique_elements.values():
            i = 0
            while (i < len(node_list) - 1):
                ...
                while (j < len(node_list)):
                    structure = []
                    if self.isMirror(node_list[i], node_list[j], structure):
                        all_unique_structures[str(structure)] = node_list[i]
                        del node_list[j]
                        #j +=1
                    else:
                        j += 1
